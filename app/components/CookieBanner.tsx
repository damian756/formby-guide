...
export default function CookieBanner() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[9999] p-4 sm:p-6">
      <div className="max-w-3xl mx-auto bg-[#1C3220] rounded-2xl shadow-2xl border border-white/10 overflow-hidden">
        {/* Gold accent line */}
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="flex items-start gap-3 mb-4">
            <div className="w-9 h-9 rounded-xl bg-[#C9A96E]/15 flex items-center justify-center flex-none mt-0.5">
              <Cookie className="w-4.5 h-4.5 text-[#C9A96E]" />
            </div>
            <div className="flex-1">
              ...
              <p className="text-white/60 text-sm mt-1 leading-relaxed">
                We use cookies to improve your experience and help us understand how visitors use the site. Analytics cookies are only set with your consent.
              </p>
            </div>
          </div>
          ...
          {showDetails && (
            <div className="mb-5 rounded-xl overflow-hidden border border-white/10 text-xs">
              ...
              {[
                ...
                { cat: "Marketing", purpose: "Affiliate tracking, ad referrals", consent: "Optional" },
                ...
                <div key={cat} className="grid grid-cols-3 px-4 py-3 border-t border-white/5 text-white/70">
                  ...
                  <span>{purpose}</span>
                  <span className={c === "Always on" ? "text-[#C9A96E]" : "text-white/50"}>{c}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <button
              onClick={acceptAll}
              className="flex-1 sm:flex-none bg-[#2E6B3E] hover:bg-[#3a8050] text-white font-bold px-6 py-2.5 rounded-full text-sm transition-colors"
            >
              Accept all
            </button>
            <button
